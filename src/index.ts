...
const walk = async (entryPath: string): Promise<string[]> => {
  ...
  const nested = await Promise.all(
    children.map((child) => walk(path.join(entryPath, child.name))),
  );
...
const matchesPatterns = (filePath: string, rootDir: string, patterns: string[]): boolean => {
...
const scanFile = async (filePath: string): Promise<Finding[]> => {
...
const getRelativePath = (rootDir: string, filePath: string): string =>
  path.relative(rootDir, filePath) || path.basename(filePath);

const shouldFail = (findings: Finding[], failOn: FailOn): number => {
  if (failOn === "none") {
    return 0;
  }
...
const printTextOutput = (findings: Finding[], rootDir: string): void => {
...
const printJsonOutput = (findings: Finding[], rootDir: string): void => {
  const payload = {
    rootDir,
...
const appendGithubSummary = async (findings: Finding[], rootDir: string): Promise<void> => {
  ...
  const lines = findings.length === 0
    ...
    : [
        ...
        ...findings.map(
          (finding) =>
            `| ${finding.severity} | ${finding.ruleId} | \`${getRelativePath(
              rootDir,
              finding.filePath,
...
const setGithubOutputs = async (findings: Finding[]): Promise<void> => {
...
const printGithubOutput = async (findings: Finding[], rootDir: string): Promise<void> => {
  if (findings.length === 0) {
  ...
  for (const finding of findings) {
    ...
    console.log(
      `::${level} file=${relativePath},line=${finding.line},title=${finding.ruleId}::${message}`,
    );
  }
...
const main = async (): Promise<void> => {
  try {
    ...
    const allFiles = (await walk(targetDir))
      ...
      .sort();
    const findings = (
      await Promise.all(allFiles.map((filePath) => scanFile(filePath)))
    )
      .flat()
